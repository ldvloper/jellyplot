<div class="flex flex-col">
    <div class="mb-1 p-5 w-full border border-primary-color rounded-xl">
        ...
        <div class="md:hidden mt-5">
            {{ $positions->links('vendor.livewire.pagination.tailwind') }}
        </div>
    </div>
    <!--Table view-->
            ...
            <div class="shadow overflow-hidden border-b border-gray-200 bg-primary-color sm:rounded-lg">
                ...
                <div class="flex mt-5 mb-2 mr-2">
                        ...
                        <h1 class="text-sm">
                            ...
                            <span class="text-md font-semibold">
                                @if($positions->total()===1)
                                    {{$positions->total()}} {{__('Position Found')}}
                                @else
                                    {{$positions->total()}} {{__('Positions Found')}}
                                @endif
                            </span>
                        </h1>
                    </div>
                    @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'create'))
                        <div class="pr-5 self-center order-last">
                            <a href="{{route('positions.create')}}" class="flex justify-end text-sm p-3 text-white bg-green-700 hover:bg-green-600 rounded-md cursor-pointer">
                                ...
                                <span class="hidden md:block"> {{__('New Position')}}</span>
                            </a>
                        </div>
                    @endif
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        @else
                            <th class="p-5">{{__('No positions found')}}</th>
                        @endif
                    ...
                    @foreach($positions as $position)
                        <tr class="group hover:bg-primary-color hover:text-white {{ $position->trashed() ? 'bg-gray-300' : '' }}">
                            <td class="flex items-center px-6 py-6 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{route('positions.show', $position)}}" class="p-2 hover:bg-white hover:text-primary-color rounded-md cursor-pointer ease-in-out"
                                   title="Show: {{$position->name}}">
                                ...
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'update'))
                                    <a href="{{route('positions.edit', $position)}}" class="p-2 hover:bg-white hover:text-primary-color rounded-md cursor-pointer ease-in-out"
                                ...
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'delete') && !$position->trashed())
                                    <form action="{{route("positions.destroy", $position->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="flex w-full justify-center p-2 hover:bg-white hover:text-primary-color rounded-md cursor-pointer ease-in-out" title="Delete: {{$position->name}}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @elseif($position->trashed())
                                    <span class="ml-1 min-vh-100w-8 self-center font-semibold">Deleted {{$position->deleted_at->diffForHumans()}}</span>
                                @endif
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex text-sm text-gray-900 group-hover:text-white">
