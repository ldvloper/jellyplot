<div class="flex flex-col">
    <div class="mb-1 p-5 w-full border border-primary-color rounded-xl">
        ...
        <div class="md:hidden mt-5">
            {{ $shifts->links('vendor.livewire.pagination.tailwind') }}
        </div>
    </div>
    <!--Table view-->
            ...
            <div class="shadow overflow-hidden border-b border-gray-200 bg-primary-color sm:rounded-lg">
                ...
                <div class="flex mt-5 mb-2 mr-2">
                    <div class="flex flex-grow p-5 w-full lg:w-3/12 text-white">
                        <h1 class="text-sm">
                            ...
                            <span class="text-md font-semibold">
                                @if($shifts->total()===1)
                                    {{$shifts->total()}} {{__('Shift Found')}}
                                @else
                                    {{$shifts->total()}} {{__('Shifts Found')}}
                                @endif
                            </span>
                        </h1>
                    </div>
                    @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'create'))
                        <div class="pr-5 self-center order-last">
                            <a href="{{route('shifts.create')}}" class="flex justify-end text-sm p-3 text-white bg-green-700 hover:bg-green-600 rounded-md cursor-pointer">
                                ...
                                <span class="hidden md:block"> {{__('New Shift')}}</span>
                            </a>
                        </div>
                    @endif
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        @else
                            <th class="p-5">{{__('No Shift found')}}</th>
                        @endif
                    ...
                    @foreach($shifts as $shift)
                        <tr class="group hover:bg-primary-color hover:text-white {{ $shift->trashed() ? 'bg-gray-300' : '' }}">
                            <td class="flex items-center px-6 py-6 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{route('shifts.show', $shift)}}" class="p-2 hover:bg-white hover:text-primary-color rounded-md cursor-pointer ease-in-out"
                                ...
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'update'))
                                    <a href="{{route('shifts.edit', $shift)}}" class="p-2 hover:bg-white hover:text-primary-color rounded-md cursor-pointer ease-in-out"
                                ...
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'delete') && !$shift->trashed())
                                    <form action="{{route("shifts.destroy", $shift->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="flex w-full justify-center p-2 hover:bg-white hover:text-primary-color rounded-md cursor-pointer ease-in-out" title="Delete: {{$shift->name}}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @elseif($shift->trashed())
                                    <span class="ml-1 min-vh-100w-8 self-center font-semibold">Deleted {{$shift->deleted_at->diffForHumans()}}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="ml-4">
                                        <div class="text-sm font-light text-gray-900 group-hover:text-white capitalize">
                                            {{$shift->name}}
                                        </div>
                                    </div>
                                </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 group-hover:text-white">
                                    <p class="font-black">{{$shift->total_hours}} hours</p>
                                </div>
                            </td>
                        </tr>
                    @endforeach
