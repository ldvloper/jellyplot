<div class="flex flex-col">
    <div class="mb-1 p-5 bg-transparent w-full border border-primary-color rounded-xl">
        <h1 class="text-gray-700 dark:text-gray-300 text-xl font-bold mb-8">Filters</h1>
        <div class="grid gap-4 grid-cols-1 md:grid-cols-2">
            <div>
                <label for="customer" class="text-sm">
                    {{__('Customer:')}}
                </label>
            ...
            <div>
                    <div>
                        ...
                        <input id="search" wire:model="search" type="search" placeholder="{{__('Project Name')}}" class="w-full dark:text-white dark:bg-gray-900 focus:ring-primary-color focus:border-primary-color rounded-md sm:text-sm border-gray-300 dark:border-gray-700">
                    </div>
    ...
    @if($projects->links('vendor.livewire.pagination.tailwind'))
            ...
            <div class="shadow overflow-hidden border-b border-gray-200 bg-primary-color sm:rounded-lg">
                ...
                <div class="flex mt-5 mb-2 mr-2">
                    <div class="flex flex-grow px-5 w-full lg:w-3/12 text-white">
                        <h1 class="text-sm">
                            <span class="text-md">{{__('Search')}}:</span>
                            @if($search)
                                {{$search}}
                            @else
                                {{__('All')}}
                    ...
                    @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'create'))
                        <div class="pr-5 self-center order-last">
                            <a href="{{route('projects.create')}}" class="flex justify-end text-sm p-3 text-white bg-green-700 hover:bg-green-600 rounded-md cursor-pointer">
                                ...
                                <span class="hidden md:block">{{__('Add Project')}}</span>
                            </a>
                        </div>
                    @endif
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50 select-none">
                            <tr>
                                @if($projects->count() > 0)
                                    <th scope="col" class="w-1/3 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                                @foreach($projects as $index => $project)
                                    @if($index > 0 && $project->customer->id != $projects[$index - 1]->customer->id)
                                        <tr class="bg-gray-100">
                                            <td class="font-semibold text-md p-5 capitalize">{{$project->customer->name}}</td>
                                            <td class="text-xs p-5">{{__('Related Projects')}} ({{$project->customer->projects->count()}})</td>
                                            <td></td>
                                        </tr>
                                    @elseif($index == 0)
                                        <tr class="bg-gray-100">
                                        ...
                                        <tr>
                                    @endif
                                    <tr class="hover:bg-primary-color hover:text-white {{ $project->trashed() ? 'bg-gray-300' : '' }}">
                                        <td class="flex items-right py-4 text-sm font-medium px-2">
                                            <a href="{{route('projects.show', $project->id)}}" class="flex p-2 hover:bg-white hover:text-primary-color rounded-md"
                                               title="Show: {{$project->reference}}">
                                            ...
                                            @endif
                                            @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'delete') && !$project->trashed())
                                                <form action="{{route("projects.destroy", $project->id)}}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="flex w-full justify-center p-2 hover:bg-white hover:text-primary-color rounded-md rounded-md cursor-pointer ease-in-out" title="Delete: {{$project->name}}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            @elseif($project->trashed())
                                                <span class="ml-1 min-vh-100w-8 self-center font-semibold">Deleted {{$project->deleted_at->diffForHumans()}}</span>
                                            @endif
                                        </td>

                                        <td class="px-6 py-4 whitespace-nowrap cursor-pointer" x-data @click="window.location.href = '{{route('projects.show', $project->id)}}'">
                                            <div class="flex items-center">
                                                <span style="background-color:{{$project->color}}" class="border-2 border-gray-200 group-hover:bg-white mr-3 rounded-xl w-3 h-10"></span>
                                                <div class="text-sm font-medium uppercase">
                                                    {{$project->customer->name}} ({{$project->reference}})
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap cursor-pointer" x-data @click="window.location.href = '{{route('projects.show', $project->id)}}'">
