<div class="flex flex-col">
    <div class="mb-1 p-5 bg-transparent w-full border border-primary-color rounded-xl">
        ...
        <div class="grid gap-4 grid-cols-1 md:grid-cols-2">
            <div>
                <div>
                    <label for="resource" class="text-sm">
                        {{__('Resource:')}}
                    </label>
                ...
                <div class="mt-2">
                    ...
                    <input id="search" wire:model="search" type="search" placeholder="{{__('Task Name')}}" class="w-full dark:text-white dark:bg-gray-900 focus:ring-primary-color focus:border-primary-color rounded-md sm:text-sm border-gray-300 dark:border-gray-700">
                </div>
            </div>
            <div>
                <div>
                    <label for="project" class="text-sm">
                        {{__('Project:')}}
                    </label>
    ...
    @if($tasks->links('vendor.livewire.pagination.tailwind'))
        <div class="p-5 md:hidden">
            {{ $tasks->links('vendor.livewire.pagination.tailwind') }}
        </div>
    @endif
    <!--Table view-->
                ...
                <div class="flex mt-5 mb-2 mr-2">
                    <div class="flex flex-grow px-5 w-full lg:w-3/12 text-white">
                        <h1 class="text-sm">
                            ...
                            <span class="text-md font-semibold">
                                @if($tasks->total()===1)
                                    {{$tasks->total()}} {{__('Task Found')}}
                                @else
                                    {{$tasks->total()}} {{__('Tasks Found')}}
                                @endif
                            </span>
                        </h1>
                    </div>
                    @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'create'))
                        <div class="pr-5 self-center order-last">
                            <a href="{{route('tasks.create')}}" class="flex justify-end text-sm p-3 text-white bg-green-700 hover:bg-green-600 rounded-md cursor-pointer">
                                ...
                                <span class="hidden md:block">{{__('Add Task')}}</span>
                            </a>
                        </div>
                    @endif
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    @if($tasks->count() > 0)
                        <tr>
                            ...
                            @else
                                <th class="p-5">{{__('No tasks found')}}</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($tasks as $task)
                        <tr class="hover:bg-primary-color hover:text-white {{ $task->trashed() ? 'bg-gray-300' : '' }}">
                            <td class="flex items-right py-4 text-sm font-medium px-2">
                                <a href="{{route('tasks.show', $task->id)}}" class="flex p-2 hover:bg-white hover:text-primary-color rounded-md"
                                ...
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'update'))
                                    <a href="{{route('tasks.edit', $task)}}" class="flex p-2 hover:bg-white hover:text-primary-color rounded-md"
                                ...
                                @endif
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'delete') && !$task->trashed())
                                    <form action="{{route("tasks.destroy", $task->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="flex w-full justify-center p-2 hover:bg-white hover:text-primary-color rounded-md rounded-md cursor-pointer ease-in-out" title="Delete: {{$task->title}}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @elseif($task->trashed())
                                    <span class="ml-1 min-vh-100w-8 self-center font-semibold">Deleted {{$task->deleted_at->diffForHumans()}}</span>
                                @endif
                            </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <span style="background-color:{{$task->resource->color}}" class="border-2 border-gray-200 group-hover:bg-white mr-3 rounded-xl w-3 h-10"></span>
