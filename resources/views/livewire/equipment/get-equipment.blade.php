<div class="flex flex-col">
    <div class="mb-1 p-5 bg-transparent w-full border border-primary-color rounded-xl">
        ...
        <div class="grid gap-4 grid-cols-1 md:grid-cols-2">
            <div>
                ...
                <input id="search" wire:model="search" type="search" placeholder="{{__('Equipment Name')}}" class="w-full dark:text-white dark:bg-gray-900 focus:ring-primary-color focus:border-primary-color rounded-md sm:text-sm border-gray-300 dark:border-gray-700">
            </div>
    ...
    @if($equipments->links('vendor.livewire.pagination.tailwind'))
        <div class="p-5 md:hidden">
            {{ $equipments->links('vendor.livewire.pagination.tailwind') }}
        </div>
    @endif
    <!--Table view-->
                    ...
                    <div class="flex flex-grow px-5 w-full lg:w-3/12 text-white">
                        <h1 class="text-sm">
                            ...
                            <span class="text-md font-semibold">
                                {{$equipments->total()}} Equipment Found
                            </span>
                        </h1>
                    </div>
                    @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'create'))
                        <div class="pr-5 self-center order-last">
                            <a href="{{route('equipment.create')}}" class="flex justify-end text-sm p-3 text-white bg-green-700 hover:bg-green-600 rounded-md cursor-pointer">
                                ...
                                <span class="hidden md:block">{{__('Add Equipment')}}</span>
                            </a>
                        </div>
                    @endif
                </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50 select-none">
                        <tr>
                            ...
                            @else
                                <th class="p-5">{{__('No equipment found')}}</th>
                            @endif
                        </tr>
                    </thead>
                    ...
                    @foreach($equipments as $equipment)
                        <tr class="hover:bg-primary-color hover:text-white {{ $equipment->trashed() ? 'bg-gray-300' : '' }}">
                            <td class="flex items-right py-4 text-sm font-medium px-2">
                                <a href="{{route('equipment.show', $equipment->id)}}" class="flex p-2 hover:bg-white hover:text-primary-color rounded-md"
                                   title="Show: {{$equipment->name}}">
                                ...
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'update'))
                                    <a href="{{route('equipment.edit', $equipment)}}" class="flex p-2 hover:bg-white hover:text-primary-color rounded-md"
                                ...
                                @endif
                                @if(auth()->user()->hasTeamPermission(auth()->user()->currentTeam, 'delete') && !$equipment->trashed())
                                    <form action="{{route("equipment.destroy", $equipment->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="flex w-full justify-center p-2 hover:bg-white hover:text-primary-color rounded-md rounded-md cursor-pointer ease-in-out" title="Delete: {{$equipment->name}}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                @elseif($equipment->trashed())
                                    <span class="ml-1 min-vh-100w-8 self-center font-semibold">Deleted {{$equipment->deleted_at->diffForHumans()}}</span>
                                @endif
                            </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap cursor-pointer" x-data @click="window.location.href = '{{route('equipment.show', $equipment)}}'">
                                    <div class="flex items-center">
                                        <div class="text-sm font-medium">
                                            {{$equipment->version}}
                                        </div>
